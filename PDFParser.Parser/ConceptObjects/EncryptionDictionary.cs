...
public class EncryptionDictionary
{
    ...
    public EncryptionDictionary(DictionaryObject dict)
    {
        Dictionary = dict;
    }
    ...
    public NameObject Filter => Dictionary.GetAs<NameObject>("Filter");
    ...
    public NameObject? SubFilter => Dictionary.TryGetAs<NameObject>("SubFilter");
    ...
    [PdfDictionaryKey("V")] public NumericObject AlgorithmCode => Dictionary.GetAs<NumericObject>("V");
    
    //Required - number specifying which revision of the standard security handler shall be used to interpret this dictionary.
    public int Revision => (int)Dictionary.GetAs<NumericObject>("R").Value;
    
    //Required - 32 bytes if R is 4, 48 bytes long if the value of R is 6. Used in computing the file encryption key.
    public StringObject OwnerKey => Dictionary.GetAs<StringObject>("O");
    //Required - same as OKey.
    public StringObject UserKey => Dictionary.GetAs<StringObject>("U");
    
    //Security Keys required if R is 6 (PDF 2.0) used in decryption algorithm.
    public StringObject? OEKey => Dictionary.TryGetAs<StringObject>("OE");
    public StringObject? UEKey => Dictionary.TryGetAs<StringObject>("UE");
    ...
    public NumericObject PermissionFlags => Dictionary.GetAs<NumericObject>("P");
    ...
    public StringObject? EncryptedPermissionFlags => Dictionary.TryGetAs<StringObject>("Perms");

    //Optional - meaningful when V is 4 or 5. Indicates whether the document-level metadata stream shall be encrypted.
    //Default value is true.
    public BooleanObject? EncryptMetadata => Dictionary.TryGetAs<BooleanObject>("EncryptMetadata");
    ...
    public NumericObject? Length => Dictionary.TryGetAs<NumericObject>("Length");
    [PdfDictionaryKey("CF")] public DictionaryObject? CryptFiltersDictionary => Dictionary.TryGetAs<DictionaryObject>("CF"); //meaningful only when the value of V is 4 or 5. 
    [PdfDictionaryKey("StmF")] public NameObject? StreamFilterKey => Dictionary.TryGetAs<NameObject>("StmF"); //meaningful only when the value of V is 4 or 5.
    [PdfDictionaryKey("StrF")] public NameObject? StringFilterKey => Dictionary.TryGetAs<NameObject>("StrF"); //Meaningful only when the value of V is 4 or 5.
    [PdfDictionaryKey("EFF")]
    public NameObject? EmbeddedFilterKey => Dictionary.TryGetAs<NameObject>("EFF"); //Meaningful only when the value of V is 4.
