...
public class EncryptionDictionary
{
    ...
    public NameObject Filter => _dict.GetAs<NameObject>("Filter");
    ...
    public NameObject? SubFilter => _dict.TryGetAs<NameObject>("SubFilter");
    ...
    [PdfDictionaryKey("V")] public NumericObject AlgorithmCode => _dict.GetAs<NumericObject>("V");
    
    //Required - number specifying which revision of the standard security handler shall be used to interpret this dictionary.
    public NumericObject Revision => _dict.GetAs<NumericObject>("R");
    
    //Required - 32 bytes if R is 4, 48 bytes long if the value of R is 6. Used in computing the file encryption key.
    public StringObject OKey => _dict.GetAs<StringObject>("O");
    //Required - same as OKey.
    public StringObject UKey => _dict.GetAs<StringObject>("U");
    
    //Security Keys required if R is 6 (PDF 2.0) used in decryption algorithm.
    public StringObject? OEKey => _dict.TryGetAs<StringObject>("OE");
    public StringObject? UEKey => _dict.TryGetAs<StringObject>("UE");
    ...
    public NumericObject PermissionFlags => _dict.GetAs<NumericObject>("P");
    ...
    public StringObject? EncryptedPermissionFlags => _dict.TryGetAs<StringObject>("Perms");

    //Optional - meaningful when V is 4 or 5. Indicates whether the document-level metadata stream shall be encrypted.
    //Default value is true.
    public BooleanObject? EncryptMetadata => _dict.TryGetAs<BooleanObject>("EncryptMetadata");
    ...
    public NumericObject? Length => _dict.TryGetAs<NumericObject>("Length");
    [PdfDictionaryKey("CF")] public DictionaryObject? CryptFiltersDictionary => _dict.TryGetAs<DictionaryObject>("CF"); //meaningful only when the value of V is 4 or 5. 
    [PdfDictionaryKey("StmF")] public NameObject? StreamFilterKey => _dict.TryGetAs<NameObject>("StmF"); //meaningful only when the value of V is 4 or 5.
    [PdfDictionaryKey("StrF")] public NameObject? StringFilterKey => _dict.TryGetAs<NameObject>("StrF"); //Meaningful only when the value of V is 4 or 5.
    [PdfDictionaryKey("EFF")]
    public NameObject? EmbeddedFilterKey => _dict.TryGetAs<NameObject>("EFF"); //Meaningful only when the value of V is 4.
